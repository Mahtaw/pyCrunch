...
Prior_std = np.logspace(-16, 0) #[1,.1,.01,.001,.0001,.00001]
RR = np.logspace(-6,2)#[0,.5**3,.5**2,.5]
errors = []
import itertools
allcombos = itertools.product(Prior_std, RR)
for prior_std, R in allcombos:
    print(prior_std, R)
    try:
        ...
        errors.append((prior_std,R,'Yay'))
    except Exception as e:
        errors.append((prior_std,R,e))
